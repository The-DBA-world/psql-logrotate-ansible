...
1. **Setup Directory Structure**:
   ```bash
   mkdir -p logrotate_config_deployment
   ```
2. **Ansible Playbooks**:
...
3. **Run the Playbook**:
   ```bash
   ansible-playbook -l hostname pgsql-06-logrotate-config.yml
   ```
...
2. **Dry-Run Logrotate**:
   ```bash
   /usr/sbin/logrotate -d /etc/logrotate.d/postgresql
   ```
   - Ensures no syntax errors and shows what `logrotate` will do.
3. **Force Rotation**:
   ```bash
   /usr/sbin/logrotate -f /etc/logrotate.d/postgresql
   ```
   - Manually rotates the logs to verify behavior.
4. **Check Permissions**:
   ```bash
   ls -l /data/test/postgres/15/pg_dbtest_anil/log/
   ```
